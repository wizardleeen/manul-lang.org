...
# -----------------------------------------------------------------------------
# 6.5 Create Restart Utility Script
# -----------------------------------------------------------------------------
RESTART_SCRIPT="${INSTALL_DIR}/bin/manul-restart"
printf "Creating utility script: ${BLUE}${RESTART_SCRIPT}${NC}...\n"

cat > "$RESTART_SCRIPT" <<EOF
#!/bin/sh
# manul-restart
# Auto-generated by Manul Installer to restart the background service.

printf "${BLUE}Restarting manul-server...${NC}\n"
EOF

# Append OS-specific restart commands
if [ "$OS" = "Darwin" ]; then
    # Re-derive path logic just for the script content
    if [ "$IS_ROOT" -eq 1 ]; then
        TARGET_PLIST="/Library/LaunchDaemons/com.manul.server.plist"
    else
        TARGET_PLIST="\$HOME/Library/LaunchAgents/com.manul.server.plist"
    fi
    echo "launchctl unload \"$TARGET_PLIST\" 2>/dev/null" >> "$RESTART_SCRIPT"
    echo "sleep 1" >> "$RESTART_SCRIPT"
    echo "launchctl load \"$TARGET_PLIST\"" >> "$RESTART_SCRIPT"

elif [ "$OS" = "Linux" ]; then
    if command -v systemctl > /dev/null; then
        if [ "$IS_ROOT" -eq 1 ]; then
            echo "systemctl restart manul-server" >> "$RESTART_SCRIPT"
        else
            echo "systemctl --user restart manul-server" >> "$RESTART_SCRIPT"
        fi
    else
        echo "echo 'Error: systemctl not found. Manual restart required.'" >> "$RESTART_SCRIPT"
        echo "exit 1" >> "$RESTART_SCRIPT"
    fi
fi

echo "printf \"${GREEN}Service restarted.${NC}\n\"" >> "$RESTART_SCRIPT"
